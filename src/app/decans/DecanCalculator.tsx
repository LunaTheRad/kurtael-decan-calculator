...
const formSchema = z.object({
  ...
  ascentMultiplier: z.coerce.number()
})
...
export const DecanCalculator = (data: { decans: Decan[] }) => {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    const d = calculate(values.day, values.hour, values.minute, values.ascentMultiplier)
    setDecans([...d])
  }

  function calculate(day: number, hour: number, minute: number, ascentMultiplier: number): Decan[] {
    ...
    data.decans.forEach((decan: Decan) => {
        decan.mana =
          1
          + getPlanetMana(decan.planet, time, sunCurrent, sunFuture, ascentMultiplier)
          - getPlanetMana(opposingPlanets[decan.planet], time, sunCurrent, sunFuture, 1)
          + getZodiacMana(decan.positionValue, sunCurrent);
        if (decan.mana > 5) decan.mana = 5
        if (decan.mana < 0) decan.mana = 0
      }
    )
  ...
  function getPlanetMana(planet: string | null, time: number, sunCurrent: Coordinates, sunFuture: Coordinates, ascentMultiplier: number) {
    ...
    else if (direction < 0) return ascentMultiplier
    else return 0
  }
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <div className="flex flex-wrap gap-4">
            ...
            <FormField
              control={form.control}
              name="ascentMultiplier"
              render={({field}) => (
                <FormItem>
                  <FormLabel>Ascent Multiplier</FormLabel>
                  <FormControl>
                    <Input placeholder="1" {...field} />
                  </FormControl>
                  <FormDescription>
                    Multiplier for mana from ascent.
                  </FormDescription>
                  <FormMessage/>
                </FormItem>
              )}
